...
func main() {
	...
	err := mongodb.Insert(Database, Collection, data)
	if err != nil {
	...
	var result Data
	err = mongodb.FindOne(Database, Collection, bson.M{"_id": bson.ObjectIdHex("5b3db2334d661ff46ee14b9c")}, nil, &result)
	fmt.Println("find one with all fields", result)

	// find one without id field
	var result1 Data
	err = mongodb.FindOne(Database, Collection, bson.M{"_id": bson.ObjectIdHex("5b3db2334d661ff46ee14b9c")}, bson.M{"_id": 0}, &result1)
	fmt.Println("find one without id field", result1)

	//find all documents
	var allResult []Data
	err = mongodb.FindAll(Database, Collection, nil, nil, &allResult)
	fmt.Println("find all docs", allResult)

	// find all documents with query and selector
	var allResult1 []Data
	err = mongodb.FindAll(Database, Collection, bson.M{"title": "博客的标题 1"}, bson.M{"_id": 0}, &allResult1)
	fmt.Println("find all docs with query and selector", allResult1)

	//find documents with page and limit
	var resultWithPage []Data
	err = mongodb.FindPage(Database, Collection, 0, 4, nil, bson.M{"_id": 0}, &resultWithPage)
	fmt.Println("find docs with page and limit", resultWithPage)

	//find the cursor
	var iterAll []Data
	iter := mongodb.FindIter(Database, Collection, nil)
	err = iter.All(&iterAll)
	fmt.Println("find cursor ", iterAll)

	//update one document
	err = mongodb.Update(Database, Collection, bson.M{"_id": bson.ObjectIdHex("5b3db2334d661ff46ee14b9c")}, bson.M{"$set": bson.M{
		"title": "更新后的标题",
		"des":   "更新后的描述信息",
		"date":  time.Now(),
	}})
	...
	//delete one docment
	err = mongodb.Remove(Database, Collection, bson.M{"_id": bson.ObjectIdHex("5b3db2334d661ff46ee14b99")})
	if err != nil {
		fmt.Println("remove one doc error", err)
	}

	//upsert the docment
	err = mongodb.Upsert(Database, Collection, bson.M{"title": "Title Upsert"}, bson.M{"$set": bson.M{
		"des":     "描述Upsert",
		"date":    time.Now(),
		"Content": "内容Upsert",
	}})
	...
	insertResult, _ := mongodb.BulkInsert(Database, Collection, d1, d2)
	...
	updateResult, _ := mongodb.BulkUpdate(Database, Collection, up1, up2, up3, up4)
	...
	updateAllResult, _ := mongodb.BulkUpdateAll(Database, Collection, up5, up6)
