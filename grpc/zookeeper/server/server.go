...
func (zkServer) Register(c context.Context, r *pb.RegisterRequest) (*pb.RegisterResponse, error) {
	logger.Info("register from ", r.NodeIp+":"+r.NodePort, " --- ", r.NodeName)

	manager := Manager.GetIns()
	newNode := Manager.Node{NodeName: r.NodeName, NodeIp: r.NodeIp, NodePort: r.NodePort}
...
func (zkServer) Heartbeat(c context.Context, r *pb.HeartbeatRequest) (*pb.HeartbeatResponse, error) {
	logger.Info("heartbeat from ", r.NodeIp+":"+r.NodePort, " --- ", r.NodeName)
...
func (zkServer) UserCheckIn(c context.Context, r *pb.UserCheckInRequest) (*pb.UserCheckInResponse, error) {
	...
	// 参数校验
	if !paramCheck(r.UserId, r.NodeName) {
...
func (zkServer) FindUser(c context.Context, r *pb.FindUserRequest) (*pb.FindUserResponse, error) {
	logger.Info("FindUser from ", r.NodeName, " --- ", r.UserId)
...
func paramCheck(userId string, nodeName string) bool {
	return strings.Trim(userId, " ") != "" && strings.Trim(nodeName, " ") != ""
}
...
func InitGRPC() {
	...
	fmt.Println("gRPC server listen on " + ZKgrpcAddr)
	go s.Serve(listen)
}
